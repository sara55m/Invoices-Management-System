            ...
            <div class="nav nav-item  navbar-nav-right ml-auto">
                    ...
                    <div class="dropdown-menu">
                        <div class="menu-header-content bg-primary text-right">
                            <div class="d-flex">
                                ...
                                <span class="badge badge-pill badge-warning mr-auto my-auto float-left"><a href="/MarkAsRead_all">{{ __('messages.mark_all') }}</a></span>
                            </div>
                            <p class="dropdown-title-text subtext mb-0 text-white op-6 pb-0 tx-12 ">
                                <h6 style="color: yellow" id="notifications_count">
                                    {{ __('messages.you_have') }} {{ Auth::user()->unreadNotifications->count() }} {{ __('messages.unread_notifications') }}
                                </h6>
                                </p>
                        </div>
                        <div id="unreadNotifications">
                                ...
                                <div class="main-notification-list Notification-scroll">
                                        ...
                                        <div class="mr-3">
                                            <a class="d-flex p-3 border-bottom" href="{{url('invoicesDetails')}}/{{$notification->data['id'] }}"><h5 class="notification-label mb-1">{{$notification->data['title'] }} by {{$notification->data['user'] }}</h5></a>
                                            <div class="notification-subtext">{{$notification->created_at->diffForHumans()}}</div>
                                        </div>
                                    <span class="badge badge-pill badge-warning mr-auto my-auto float-right"><a href="/MarkAsRead/{{$notification->id}}">{{ __('messages.mark') }}</a></span>
                                </div>
                                @endforeach
                        </div>
                    </div>
                ...
                <div class="dropdown main-profile-menu nav nav-item nav-link">
                    ...
                    <div class="dropdown-menu">
                        ...
                        <a class="dropdown-item" href=""><i class="bx bx-user-circle"></i>{{__('messages.Profile')}}</a>
                        <a class="dropdown-item" href=""><i class="bx bx-cog"></i>{{__('messages.EditProfile')}}</a>
                        <a class="dropdown-item" href="{{ route('logout') }}"
                        onclick="event.preventDefault();document.getElementById('logout-form').submit();"><i
                        class="bx bx-log-out"></i>{{__('messages.SignOut')}}</a>
